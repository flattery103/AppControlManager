...
class Release0180RuleWorkerTests(unittest.TestCase):
    ...
    def test_staging_precedes_per_job_worker_write_access(self):
        ...
        self.assertIn("RuleWorkerProvisioner.GrantJobAccess(jobDirectory, stagedInput, unpublishedRequestPath)", setup)
        ...
        self.assertLess(setup.index("File.Copy(sourcePath, stagedInput"), setup.index("RuleWorkerProvisioner.GrantJobAccess(jobDirectory, stagedInput, unpublishedRequestPath)"))
        self.assertLess(setup.index("WriteJsonAtomicAsync(unpublishedRequestPath"), setup.index("RuleWorkerProvisioner.GrantJobAccess(jobDirectory, stagedInput, unpublishedRequestPath)"))
        self.assertLess(setup.index("RuleWorkerProvisioner.GrantJobAccess(jobDirectory, stagedInput, unpublishedRequestPath)"), setup.index("File.Move(unpublishedRequestPath, requestPath, false)"))
