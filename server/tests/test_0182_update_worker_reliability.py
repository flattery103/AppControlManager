...
class Release0182UpdateWorkerReliabilityTests(unittest.TestCase):
    ...
    def test_newer_reported_version_cancels_stale_older_activation_latch(self):
        server_dir = ROOT / "server"
        if str(server_dir) not in sys.path:
            sys.path.insert(0, str(server_dir))
        import app as app_module

        fd, db_path = tempfile.mkstemp(prefix="acm-rc11-stale-latch-", suffix=".db")
        os.close(fd); os.unlink(db_path)
        old_path = app_module.DB_PATH; app_module.DB_PATH = db_path
        self.addCleanup(setattr, app_module, "DB_PATH", old_path)
        self.addCleanup(lambda: os.path.exists(db_path) and os.unlink(db_path))
        app_module.init_db(); now = app_module.utcnow()
        with sqlite3.connect(db_path) as conn:
            conn.row_factory = sqlite3.Row
            conn.execute("INSERT INTO organizations(id,name,slug,status,created_at) VALUES(99,'Org','org-rc11','active',?)", (now,))
            conn.execute("INSERT INTO devices(id,hostname,device_key_hash,agent_version,desired_agent_version,update_status,created_at,organization_id) VALUES('d1','pc1','x','1.0.0-rc.10','1.0.0-rc.11','installed',?,99)", (now,))
            old_release = conn.execute("INSERT INTO agent_releases(version,channel,file_name,file_path,sha256,size_bytes,active,created_at) VALUES('1.0.0-rc.8','stable','8.zip','/tmp/8.zip',?,1,1,?)", ('A' * 64, now)).lastrowid
            next_release = conn.execute("INSERT INTO agent_releases(version,channel,file_name,file_path,sha256,size_bytes,active,created_at) VALUES('1.0.0-rc.11','stable','11.zip','/tmp/11.zip',?,1,1,?)", ('B' * 64, now)).lastrowid
            conn.execute("INSERT INTO agent_deployments(release_id,scope_type,scope_id,organization_id,rollout_percent,active,created_at) VALUES(?,'device','d1',99,100,1,?)", (next_release, now))
            conn.execute("INSERT INTO agent_update_history(device_id,release_id,from_version,target_version,status,created_at) VALUES('d1',?,'1.0.0-rc.7','1.0.0-rc.8','installing',?)", (old_release, now))
            conn.commit()
            app_module.refresh_device_update_target(conn, 'd1')
            history = conn.execute("SELECT status,completed_at FROM agent_update_history WHERE release_id=?", (old_release,)).fetchone()
            queued = conn.execute("SELECT COUNT(*) FROM commands WHERE command_type='update_agent' AND status='pending'").fetchone()[0]
        self.assertEqual('canceled', history['status'])
