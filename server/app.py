...
app = FastAPI(title="AppControl Manager Server", version="1.0.0-rc.13")
...
def _launch_server_update() -> str:
    ...
    try:
        result = subprocess.run(
            [
                'systemd-run', f'--unit={SERVER_UPDATE_UNIT}', '--no-block', '--collect', '--property=Type=oneshot',
                '/bin/bash', '-lc', command,
            ],
            capture_output=True,
            text=True,
...
class BackgroundWorkSummaryIn(BaseModel):
    ...
    def valid_status(cls, value: str) -> str:
        allowed = {
            "queued", "processing", "ready", "installed", "superseded", "expired",
            "skipped_ephemeral", "needs_attention", "failed",
        }
...
def nav(principal: Optional[Principal] = None) -> str:
    ...
    return (
        ...
        + "</div><div class='side-footer'>Server 1.0.0-rc.13</div></aside>"
    )
...
@app.get("/health")
def health():
    return {"ok": True, "version": "1.0.0-rc.13"}
