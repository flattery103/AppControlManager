# AppControl Manager 1.0.0 Administrator Guide
...
Create a deployment in the paused state, assign only the test group, then activate it. Confirm version `1.0.0`, valid signatures, service and Rule Worker health, and successful heartbeats before expanding the group. If an endpoint stops reporting, follow `ENDPOINT-SERVICE-RECOVERY.md` before assigning another update.
