# AppControl Manager 1.0.0-rc.8 Backup, Restore, and Rollback

Create a database backup before installing the release candidate and before changing a pilot group to Enforcement.

```bash
sudo /root/AppControlManager/server/backup-server.sh /root/appcontrol-manager-before-1.0.0-rc.8.db
```
...
```bash
sudo /root/AppControlManager/server/restore-server.sh /root/appcontrol-manager-before-1.0.0-rc.8.db --confirm
```
