...
Write-Host 'Preparing AppControl Manager 0.18.3 binaries for the current enforcement policy...'
if((Test-Path $trustHelper) -and (Test-Path $basePolicy)) {
    try {
        $trustResult=& $trustHelper -FilePath $binaryList -Name 'AppControl Manager 0.18.3 Core Binaries' -AsObject -AlreadyExpanded
        ...
        throw "Could not pre-authorize the 0.18.3 binaries: $($_.Exception.Message)"
    }
}
...
if(-not [string]::IsNullOrWhiteSpace($preauthPolicyId)) {
    [ordered]@{ version='0.18.3'; preauth_policy_id=$preauthPolicyId } | ConvertTo-Json | Set-Content -LiteralPath "$programData\Updates\current-update.json" -Encoding UTF8
}
...
Write-Host 'AppControl Manager endpoint upgraded to 0.18.3.' -ForegroundColor Green
