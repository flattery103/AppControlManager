...
Write-Host 'Preparing AppControl Manager 1.0.0-rc.5 binaries for the current enforcement policy...'
if((Test-Path $trustHelper) -and (Test-Path $basePolicy)) {
    try {
        $trustResult=& $trustHelper -FilePath $binaryList -Name 'AppControl Manager 1.0.0-rc.5 Core Binaries' -AsObject -AlreadyExpanded
        ...
        throw "Could not pre-authorize the 1.0.0-rc.5 binaries: $($_.Exception.Message)"
    }
}
...
if(-not [string]::IsNullOrWhiteSpace($preauthPolicyId)) {
    [ordered]@{ version='1.0.0-rc.5'; preauth_policy_id=$preauthPolicyId } | ConvertTo-Json | Set-Content -LiteralPath "$programData\Updates\current-update.json" -Encoding UTF8
}
...
Write-Host 'AppControl Manager endpoint upgraded to 1.0.0-rc.5.' -ForegroundColor Green
