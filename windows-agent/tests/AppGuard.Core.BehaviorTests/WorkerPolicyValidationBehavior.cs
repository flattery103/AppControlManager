...
internal static class WorkerPolicyValidationBehavior
{
    ...
    public static void Run()
    {
        ...
        AcceptsAndPrunesUnrelatedCompanionSigner();
    ...
    private static void AcceptsAndPrunesUnrelatedCompanionSigner()
    {
        var path = WriteFixture(ProductPolicyWithUnrelatedCompanionSigner);
        try
        {
            ...
            if (!mode.Equals("product", StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException("cross-signed ProductName policy derived the wrong rule mode");
            var normalized = File.ReadAllText(path);
            if (normalized.Contains("ID_SIGNER_COMPANION", StringComparison.Ordinal))
                throw new InvalidOperationException("unvalidated companion signer remains publishable");
            if (!normalized.Contains("ID_SIGNER_CONTOSO", StringComparison.Ordinal))
                throw new InvalidOperationException("validated signer was removed with its companion");
        }
        finally { File.Delete(path); }
    }
    ...
    private const string ProductPolicyWithUnrelatedCompanionSigner = """
        <SiPolicy xmlns="urn:schemas-microsoft-com:sipolicy" PolicyType="Base Policy">
          <VersionEx>10.0.0.0</VersionEx><BasePolicyID>{22222222-2222-2222-2222-222222222222}</BasePolicyID><PolicyID>{22222222-2222-2222-2222-222222222222}</PolicyID>
          <PlatformID>{2E07F7E4-194C-4D20-B7C9-6F44A6C5A234}</PlatformID>
          <Rules><Rule><Option>Enabled:UMCI</Option></Rule><Rule><Option>Enabled:Audit Mode</Option></Rule><Rule><Option>Enabled:Unsigned System Integrity Policy</Option></Rule><Rule><Option>Enabled:Advanced Boot Options Menu</Option></Rule></Rules>
          ...
          <Signers>
            ...
            <Signer ID="ID_SIGNER_COMPANION"><CertRoot Type="TBS" Value="FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"/><CertPublisher Value="Catalog Companion"/><FileAttribRef RuleID="ID_FILEATTRIB_PRODUCT"/></Signer>
          </Signers>
          <SigningScenarios><SigningScenario Value="12" ID="ID_SIGNINGSCENARIO_WINDOWS"><ProductSigners><AllowedSigners><AllowedSigner SignerId="ID_SIGNER_CONTOSO"/><AllowedSigner SignerId="ID_SIGNER_COMPANION"/></AllowedSigners></ProductSigners></SigningScenario></SigningScenarios>
          <UpdatePolicySigners/><CiSigners><CiSigner SignerId="ID_SIGNER_CONTOSO"/><CiSigner SignerId="ID_SIGNER_COMPANION"/></CiSigners><HvciOptions>0</HvciOptions>
        </SiPolicy>
        """;
